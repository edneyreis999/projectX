...
class MinaKravensDomain {
  ...
  protected _gerarNumeroAleatorio() {
    return Math.random();
  }

  executarMineracao(request: MineracaoRequest): MineracaoResponse {
    ...
    if (this.isQuestCompleta(kravensJaColetados)) {
      const response: MineracaoResponse = {
        tipo: 'Pedra',
        questCompleta: true,
    ...
    const response: MineracaoResponse = obteuKraven ? { tipo: 'Kraven', ...responseData } : { tipo: 'Pedra', ...responseData };
    this.assertValidResponse(response);
    return response;
  }
  ...
  assertValidRequest(req: any): asserts req is MineracaoRequest {
  ...
  assertValidResponse(res: any): asserts res is MineracaoResponse {
    const tipoOK = res.tipo === 'Kraven' || res.tipo === 'Pedra';
    if (!tipoOK) throw new Error('tipo deve ser "Kraven" ou "Pedra"');
    if (typeof res.questCompleta !== 'boolean') throw new Error('questCompleta deve ser um boolean');
    if (typeof res.deveAtivarRachadura !== 'boolean') throw new Error('deveAtivarRachadura deve ser um boolean');
    if (typeof res.pilhasRestantes !== 'number') throw new Error('pilhasRestantes deve ser um número');
    if (typeof res.kravensColetados !== 'number' || res.kravensColetados < 0) throw new Error('kravensColetados deve ser um número não negativo');
    if (typeof res.chanceCalculada !== 'number' || res.chanceCalculada < 0 || res.chanceCalculada > 100) throw new Error('chanceCalculada deve ser um número entre 0 e 100');
  }
}

export default MinaKravensDomain;
