...
TAA.cp.functions.parsePose = function (rawPose, isEvent) {
  ...
  if (isEvent) pose['Index'] = rawPose['Spritesheet Index'];
  else pose['Index'] = parseInt(rawPose['Spritesheet Index']);
  if ((!isEvent && isNaN(pose['Index'])) || pose['Spritesheet'] === '') {
    pose['Index'] = isEvent ? '0' : 0;
    pose['Spritesheet'] = '';
  }
  pose['Blink'] = rawPose['Blink Spritesheet'];
  if (isEvent) pose['Blink Index'] = rawPose['Blink Index'];
  else pose['Blink Index'] = parseInt(rawPose['Blink Index']);
  ...
  if (p !== undefined && p.length > 0) {
    for (var k = 0; k < p.length; k++) {
      var index = p[k];
      if (index.match(/(?:d[12346789])?i?[0-9]+/i)) pose['Pattern'].push(index);
    }
  }
...
TAA.cp.functions.loadBalloonPoses = function (array, isEvent) {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    if (isEvent) resultArray[balloonId]['Index'] = element['Spritesheet Index'];
    else resultArray[balloonId]['Index'] = parseInt(element['Spritesheet Index']);
    var p = JSON.parse(element['Pattern']);
    resultArray[balloonId]['Pattern'] = [];
    if (p !== undefined && p.length > 0) {
      for (var k = 0; k < p.length; k++) {
        var index = p[k];
        if (index.match(/(?:d[12346789])?i?[0-9]+/i)) resultArray[balloonId]['Pattern'].push(index);
      }
    }
...
TAA.cp.functions.loadActorPoseParameters = function (array) {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    if (element['Actor Balloon Poses'] !== undefined && element['Actor Balloon Poses'] !== '' && element['Actor Balloon Poses'].length > 0) {
      ...
      if (balloonPoses !== undefined && balloonPoses.length > 0) resultArray[actorId].Balloons = balloonPoses.slice();
    }
  }
  return resultArray;
};
...
TAA.cp.functions.loadEventPoses = function (obj) {
  ...
  if (balloons !== undefined && balloons !== '' && balloons !== '[]') {
    ...
    if (balloonPoses !== undefined && balloonPoses.length > 0) result.Balloons = balloonPoses.slice();
  }
  return result;
};
...
if (Parameters['Actor Poses'] === undefined || Parameters['Actor Poses'] === '' || Parameters['Actor Poses'].length <= 0) TAA.cp.Parameters.Actor = [];
else TAA.cp.Parameters.Actor = TAA.cp.functions.loadActorPoseParameters(JSON.parse(Parameters['Actor Poses']));

TAA.cp.Parameters.Event = TAA.cp.Parameters.Event || {};
if (Parameters['Pose Templates'] === undefined || Parameters['Pose Templates'] === '' || Parameters['Pose Templates'].length <= 0) TAA.cp.Parameters.Event = {};
else TAA.cp.Parameters.Event = TAA.cp.functions.loadEventParameters(JSON.parse(Parameters['Pose Templates']));

TAA.cp.Parameters.Default = TAA.cp.Parameters.Default || {};
if (Parameters['Default Event Poses'] === undefined || Parameters['Default Event Poses'] === '' || Parameters['Default Event Poses'].length <= 0) TAA.cp.Parameters.Default = {};
else TAA.cp.Parameters.Default = TAA.cp.functions.loadEventPoses(JSON.parse(Parameters['Default Event Poses']));
...
Game_System.prototype.getPoseCharacterById = function (id) {
  ...
  else if (id < 0) char = $gamePlayer.followers().follower(Math.abs(id) - 1);
  else char = $gameMap._events[id];
  return char;
};
...
Game_System.prototype.deleteEventPoseData = function (mapId, eventId) {
  ...
  if (this._cpSavedEventPoses[key] !== undefined && this._cpSavedEventPoses[key] !== null) delete this._cpSavedEventPoses[key];
};
...
if (Utils.RPGMAKER_NAME === 'MZ') {
  ...
  PluginManager.registerCommand(TAA.cp.PluginName, 'idlePose', args => {
    ...
    if (context === 'party') {
      if (command) $gameSystem.enablePartyIdle();
      else $gameSystem.disablePartyIdle();
      return;
    }
    if (context === 'actor') {
      charId = --charId * -1;
    ...
    if (command) $gameSystem.enableIdle(charId);
    else $gameSystem.disableIdle(charId);
  });
  ...
  PluginManager.registerCommand(TAA.cp.PluginName, 'balloonPose', args => {
    ...
    if (command) $gameSystem.enableBalloonPoses(charId);
    else $gameSystem.disableBalloonPoses(charId);
  });
  ...
  PluginManager.registerCommand(TAA.cp.PluginName, 'resetPose', args => {
    ...
    if (resetContext === 'Pose Library') {
      ...
      if (isNaN(balloonId) || balloonId <= 0) $gameSystem.resetBalloonPose(charId, balloonId);
      else $gameSystem.resetBalloonPoseLibrary(charId);
    }
  });
...
ImageManager.addCharToCache = function (file) {
  if (Utils.RPGMAKER_NAME === 'MZ') this.loadBitmap('img/characters/', file);
  else this.loadBitmap('img/characters/', file, undefined, false);
  if (this._discardCharQueue.contains(file)) this._discardCharQueue.splice(this._discardCharQueue.indexOf(file), 1);
};

ImageManager.removeCharFromCache = function (file, hue) {
  if (this.isCharCached()) {
    if (Utils.RPGMAKER_NAME === 'MZ') this._cache['img/characters/' + file].destroy();
    else {
      var key = this._generateCacheKey('img/characters/' + this.mvEncodeURI(file) + '.png', hue);
      if (this._imageCache._items[key]) this._imageCache._items[key].destroy();
    }
  }
};
...
ImageManager.isCharCached = function (file, hue) {
  if (Utils.RPGMAKER_NAME === 'MZ') {
    ...
    if (!this._cache[url]) return false;
    return true;
  }
  if (!hue) hue = 0;
  var key = this._generateCacheKey('img/characters/' + this.mvEncodeURI(file) + '.png', hue);
  if (this._imageCache.get(key)) return true;
  return false;
};
...
ImageManager.registerCommonFiles = function (file, type) {
  if (['Game_Player', 'Game_Follower'].contains(type)) this._partyFiles.push(file);
  if (['Game_Event'].contains(type)) this._eventIndex.push(file);
};

ImageManager.clearCharDiscardQueue = function () {
  var cache = this._imageCache;
  if (Utils.RPGMAKER_NAME === 'MZ') cache = this._cache;
...
Game_CharacterBase.prototype.charImgExists = function (file) {
  if (TAA.cp.Parameters.Global.CheckFiles !== true || file === '') return true;
...
Game_CharacterBase.prototype.setAnimatedPose = function (poseKey, cycles) {
  ...
  if ((this._cpCurrentPose = poseKey)) {
    ...
    if (!isNaN(cycles) && cycles > 0) this._cpPoseCycles = cycles;
  }
...
Game_CharacterBase.prototype.disableIdlePose = function () {
  ...
  if (this.isIdlePose()) this.restorePreviousPose();
};
...
Game_CharacterBase.prototype.getPoseObj = function (poseKey) {
  var balloonId = 0;
  if (poseKey !== undefined && poseKey.match(/^@balloon([0-9]+)/)) balloonId = parseInt(RegExp.$1);
  if (balloonId === 0 || !this.hasBalloonPose(balloonId)) return this._cpPoseLibrary[poseKey];
  else {
...
Game_CharacterBase.prototype.startBlink = function () {
  ...
  if (char.match(/\[d8\]/i) && this.is8DirChar()) {
    if (this._characterIndex < 4) index = index % 4;
    else index = (index % 4) + 4;
  }
...
Game_CharacterBase.prototype.isBlinking = function () {
  ...
  if (this.is8DirChar()) return this._characterName === pose['Blink'] && this._characterIndex % 4 === pose['Blink Index'] % 4;
...
Game_CharacterBase.prototype.shouldBlink = function () {
  ...
  var pose = this.currentPoseObj();
  if (pose === undefined || pose['Blink'] === '' || !this.charImgExists(pose['Blink'])) return false;
  if (!this.isBlinking()) {
    if (this._cpBlinkCounter === 0) return true;
    else return false;
  }
  return false;
};
...
Game_CharacterBase.prototype.updateBlinking = function () {
  ...
  else if (this.shouldUnblink()) this.endBlink();
  else if (this._cpBlinkCounter > 0) this._cpBlinkCounter--;
};

Game_CharacterBase.prototype.updateBalloonFrameCount = function () {
  if (this.isBalloonPosePlaying()) {
    if (this._cpBalloonFrameCount > 0) this._cpBalloonFrameCount--;
    else this.endBalloonPose();
  }
};

TAA.cp.alias.CharBase.updateAnimation = Game_CharacterBase.prototype.updateAnimation;
Game_CharacterBase.prototype.updateAnimation = function () {
  if (this.hasCustomPattern(this._cpCurrentPose) && this.cpCustomPatternNeeded()) this.updateCpAnimation();
  else TAA.cp.alias.CharBase.updateAnimation.call(this);
  this.updateDashPose();
  this.updateIdlePose();
  this.updateBlinking();
...
Game_CharacterBase.prototype.updateCpPosePattern = function () {
  ...
  } else this._pattern = parseInt(pattern);
...
Game_CharacterBase.prototype.updatePattern = function () {
  ...
  } else TAA.cp.alias.CharBase.updatePattern.call(this);
};
...
Game_CharacterBase.prototype.updateAnimationCount = function () {
  if (this.hasCustomPattern(this.currentPoseKey())) {
    this.updatePattern();
  } else TAA.cp.alias.CharBase.updateAnimationCount.call(this);
};
...
Game_CharacterBase.prototype.resetPattern = function () {
  ...
  if (this.hasCustomPattern(this.currentPoseKey()) && this.currentPoseObj() !== undefined) {
    this.resetCpPattern();
  } else TAA.cp.alias.CharBase.resetPattern.call(this);
};
...
Game_CharacterBase.prototype.isOriginalPattern = function () {
  ...
  } else return TAA.cp.alias.CharBase.isOriginalPattern.call(this);
};
...
Game_CharacterBase.prototype.pattern = function () {
  ...
  if (frames !== 4) return this._pattern < frames - 1 ? this._pattern : 1;
...
Game_CharacterBase.prototype.update8DirIndex = function () {
  if (this._direction % 2 === 1 && this._characterIndex / 4 < 1) this._characterIndex += 4;
  else if (this.is8DirChar() && this._direction % 2 === 0 && this._characterIndex / 4 >= 1) this._characterIndex -= 4;
};
...
Game_CharacterBase.prototype.moveDiagonally = function (horz, vert) {
  ...
  if (TAA.cp.Parameters.Global.Enable8Dir && this.is8DirChar()) {
    if (horz === 4) {
      if (vert === 8) this.setDirection(7);
      else if (vert === 2) this.setDirection(1);
      else this.setDirection(8);
    } else if (horz === 6) {
      if (vert === 8) this.setDirection(9);
      else if (vert === 2) this.setDirection(3);
      else this.setDirection(6);
    } else if (vert === 8) this.setDirection(8);
    else this.setDirection(2);
  }
  if (TAA.cp.Parameters.Global.DiagTriggerTouch) this.checkEventTriggerTouchFront(this.direction());
};

TAA.cp.alias.CharBase.canPass = Game_CharacterBase.prototype.canPass;
Game_CharacterBase.prototype.canPass = function (x, y, d) {
  if (this._direction % 2 === 0 || !this.is8DirChar() || TAA.cp.Parameters.Global.Enable8Dir !== true) return TAA.cp.alias.CharBase.canPass.call(this, x, y, d);
  else {
    switch (d) {
      case 1:
...
Game_CharacterBase.prototype.set8DirectionFromCoordinates = function (x, y) {
  if (x < 0 && y < 0) this.setDirection(1);
  else if (x > 0 && y < 0) this.setDirection(3);
  else if (x < 0 && y > 0) this.setDirection(7);
  else if (x > 0 && y > 0) this.setDirection(9);
};

TAA.cp.alias.CharBase.jump = Game_CharacterBase.prototype.jump;
Game_CharacterBase.prototype.jump = function (xPlus, yPlus) {
  TAA.cp.alias.CharBase.jump.call(this, xPlus, yPlus);
  if (this.is8DirChar()) this.set8DirectionFromCoordinates(xPlus, yPlus);
};

TAA.cp.alias.CharBase.setDirection = Game_CharacterBase.prototype.setDirection;
Game_CharacterBase.prototype.setDirection = function (d) {
  if (!this.is8DirChar() && (d % 2 === 1 || d === 5)) d = ((d + 1) % 8) + 2;
...
Game_Character.prototype.turnTowardCharacter = function (character) {
  ...
  } else TAA.cp.alias.Game_Character.turnTowardCharacter.call(this, character);
};
...
Game_Character.prototype.turnAwayFromCharacter = function (character) {
  ...
  if (this.is8DirChar() && Math.abs(sx) > 0 && Math.abs(sy) > 0) {
    this.set8DirectionFromCoordinates(sx, sy);
  } else TAA.cp.alias.Game_Character.turnAwayFromCharacter.call(this, character);
};
...
Game_Character.prototype.turnRandom = function () {
  if (this.is8DirChar()) {
    ...
    if (d === 5) d = 2;
    this.setDirection(d);
  } else TAA.cp.alias.Game_Character.turnRandom.call(this);
};
...
Game_Character.prototype.findDirectionTo = function (goalX, goalY) {
  if (TAA.cp.Parameters.Global.Enable8Dir === false) {
    ...
    if (deltaX1 < 0) {
      ...
      else if (deltaY1 > 0) return 1;
      else return 4;
    } else if (deltaX1 > 0) {
      if (deltaY1 < 0) return 9;
      else if (deltaY1 > 0) return 3;
      else return 6;
    } else if (deltaY1 > 0) return 2;
    else if (deltaY1 < 0) return 8;
...
Game_Player.prototype.refresh = function () {
  ...
  if (actor && this.isPoseAvailable(actor._cpCurrentPose)) {
    ...
    if (actor.followers().isVisible()) actor.followers().refresh();
  } else TAA.cp.alias.Game_Player.refresh.call(this);
};
...
Game_Player.prototype.getInputDirection = function () {
  if (TAA.cp.Parameters.Global.Enable8Dir === true) return Input.dir8;
...
Game_Follower.prototype.realMoveSpeed = function () {
  let speed = 4;
  if (TAA.cp.Parameters.Global.Enable8Dir) speed = $gamePlayer.realMoveSpeed();
  else {
    speed = TAA.cp.alias.Game_Follower.realMoveSpeed.call(this);
    if (this.isDashing()) speed--;
  }
  return speed;
};
...
Game_Event.prototype.loadPosesFromNotes = function () {
  ...
  if (
    notes.match(
      /<TAA_CP:\s*(?:,?noDefaults\s*|,?\s*default:[^\s<>,]+|,?\s*savePose:(?:false|true)|,?\s*dash:[^\s<>,]+|,?\s*idle:[^\s<>,]+|,?\s*lib:[^\s<>,]+(?:,\s*[^\s<>,]+)*|,?\s*balloon[s]?:[0-9]+(?:,\s*[0-9]+)*|,?\s*enable:(?:false|true))+>/i,
    )
  ) {
    ...
    if (notes.match(/<TAA_CP:[^>]*balloon[s]?:([0-9]+(?:,\s*[0-9]+)*)/i)) {
      ...
      for (var i = 0; i < keyList.length; i++) {
        ...
        if (!isNaN(b)) this.loadBalloonPoseTemplate(b);
      }
    }
...
Game_Event.prototype.loadPosesFromComments = function (poseKey) {
  ...
  while (i < pageList.length && !foundEndTag) {
    ...
    if (page.code === 108 || page.code === 408) {
      if (page.parameters[0].match(/<TAA_CP>/i)) foundStartTag = true;
      else if (page.parameters[0].match(/<\/TAA_CP>/i)) foundEndTag = true;
      else if (foundStartTag) poseComments.push(page.parameters[0]);
    }
    i++;
  }
  if (foundStartTag && poseComments.length >= 1) this.parsePoseComments(poseComments);
};
...
Game_Event.prototype.parsePoseComments = function (array) {
  ...
  for (var i = 0; i < array.length; i++) {
    if (array[i].match(/cpLoad:/i)) {
      ...
      newPoseKey = RegExp.$1;
      if (newPose['Pose Key'] !== undefined && newPose['Pose Key'] !== newPoseKey) newPose = {};
      ...
      if (array[i].match(/cpSprite:([^,]+),([^\r\n]+)/i)) {
        ...
        var indexStr = RegExp.$2;
        file = file.replace(/%{char}/gi, this.characterName());
        indexStr = indexStr.replace(/%{index}/gi, this.characterIndex()).replace(/%{pIndex}/gi, newPose['Index']);
        var index = parseInt(eval(indexStr));
        if (isNaN(index)) index = 0;
        newPose['Blink'] = file;
        ...
        if (balloonId > 0) newBalloon['Speed'] = !isNaN(speed) ? speed : 15;
        else newPose['Speed'] = !isNaN(speed) ? speed : 15;
        ...
        for (var k = 0; k < patternItems.length; k++) {
          if (balloonId > 0) newBalloon['Pattern'].push(patternItems[k]);
          else newPose['Pattern'].push(patternItems[k]);
        }
...
Game_Event.prototype.deleteCustomPose = function (poseKey) {
  if (this.isPoseAvailable(poseKey)) {
    if (this.isCurrentPose(poseKey)) this.setPose(this._cpDefaultPose);
...
Game_Event.prototype.loadPoseTemplate = function (poseKey) {
  ...
  pose['Pose Key'] = poseKey;
  pose['Spritesheet'] = reference['Spritesheet'].replace(/%{char}/gi, this.characterName());
...
Game_Event.prototype.parsePoseData = function (poseData) {
  ...
  pose['Spritesheet'] = poseData['Spritesheet'].replace(/%{char}/gi, this.characterName());
...
Game_Event.prototype.parseBalloonPoseData = function (balloonData) {
  ...
  balloon['Spritesheet'] = balloonData['Spritesheet'].replace(/%{char}/gi, this.characterName());
...
Game_Event.prototype.loadDefaultPoses = function () {
  ...
  var notes = this.event().note;
  if (notes.match(/<TAA_CP: [^>]*(?:,?noDefaults\s*)[^>]*>/i)) return;
...
Game_Event.prototype.isOriginalPattern = function () {
  if (this.hasCustomPattern(this._cpCurrentPose)) {
    var pose = this.currentPoseObj();
    return this._cpPatternCursor === Math.floor((pose.Pattern.length - 1) / 2);
  } else return TAA.cp.alias.Game_Event.isOriginalPattern.call(this);
};
...
Game_Event.prototype.resetPattern = function () {
  ...
  if (this.hasCustomPattern(this.currentPoseKey()) && this.currentPoseObj() !== undefined) {
    ...
    var pat = Math.floor((pose.Pattern.length - 1) / 2);
    this._cpPatternCursor = pat;
  } else TAA.cp.alias.Game_Event.resetPattern.call(this);
};
...
Window_Base.prototype.drawCharacter = function (characterName, characterIndex, x, y) {
  var frames = 3;
  if (characterName.match(/\[f(\d+)\]/i)) frames = parseInt(RegExp.$1);
...
Sprite_Character.prototype.characterPatternY = function () {
  if (this._character.cpSpriteDirection() > 0) return (this._character.cpSpriteDirection() - 2) / 2;
  ...
  } else return TAA.cp.alias.Sprite_Character.characterPatternY.call(this);
};
...
Game_Map.prototype.roundXWithDirection = function (x, d) {
  if (d % 2 === 1 && TAA.cp.Parameters.Global.Enable8Dir) {
    if ([1, 4, 7].contains(d)) return this.roundX(x - 1);
    else if ([3, 6, 9].contains(d)) return this.roundX(x + 1);
    else return this.roundX(x);
  } else return TAA.cp.alias.Game_Map.roundXWithDirection.call(this, x, d);
};

TAA.cp.alias.Game_Map.roundYWithDirection = Game_Map.prototype.roundYWithDirection;
Game_Map.prototype.roundYWithDirection = function (y, d) {
  if (d % 2 === 1 && TAA.cp.Parameters.Global.Enable8Dir) {
    if ([1, 4, 7].contains(d)) return this.roundX(y - 1);
    else if ([3, 6, 9].contains(d)) return this.roundX(y + 1);
    else return this.roundX(y);
  } else return TAA.cp.alias.Game_Map.roundYWithDirection.call(this, y, d);
};
